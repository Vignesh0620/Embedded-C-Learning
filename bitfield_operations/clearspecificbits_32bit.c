...
uint32_t clear_bits(uint32_t reg, uint8_t pos, uint8_t len) {
    ...
    //create a series of ones upto len
    uint32_t mask = (1U << len)-1;

    //shift the ones to the position bits
    mask = mask << pos;

    //return the and of register and the inverted mask , make all the ones prositioned to 0
    return reg & ~mask;
}
