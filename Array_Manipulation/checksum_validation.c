...
int validate_checksum(int *mem, int n) {
    ...
    int *ptr = mem,res=0; //initialize a ptr variable and result 

    for(int i=0;i<n-1;i++)
    {
        res ^= *ptr; //loop through the array and fix the xor with the elements
        ptr++; //increment pointer variable
    }
    int checksum = *ptr; //last ptr variable is the end of array which is checksum
    if(res == checksum)
        return 1;
    else
    return 0;
